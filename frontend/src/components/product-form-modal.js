...
export function showProductFormModal({
  mode = "add",
  product = null,
  onSuccess = null,
} = {}) {
    ...
    <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-8 relative animate-fade-in">
      ...
      <form id="product-form" autocomplete="off" class="space-y-5">
        <div>
          ...
          <input type="text" name="libelle" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500" value="${
            product?.libelle || ""
          }" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Description <span class="text-red-500">*</span></label>
          <textarea name="description" required rows="3" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500">${
            product?.description || ""
          }</textarea>
        </div>
        <div class="flex gap-4">
          <div class="flex-1">
            ...
            <input type="number" name="prix" min="0" step="0.01" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500" value="${
              product?.prix || ""
            }" />
          </div>
          <div class="flex-1">
        ...
        <div class="flex justify-end gap-2 mt-6">
          ...
          <button type="submit" class="px-6 py-2 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold hover:from-indigo-700 hover:to-purple-700 transition-all">${
  ...
  (async () => {
    ...
    try {
      ...
      // On inject le csrf dans le formulaire dans un input hidden
  ...
  imageInput.addEventListener("change", async () => {
    ...
    if (
      selectedImages.length + newFiles.length + (existingImages?.length || 0) >
      3
    ) {
    ...
    .addEventListener("submit", async (e) => {
      ...
      const formValues = {
        ...
        imagesToRemove:
          mode === "edit"
            ? JSON.stringify(
                product?.image_url?.filter(
                  (img, idx) => !existingImages.includes(img)
                )
              )
            : undefined,
      ...
      try {
        ...
        selectedImages.forEach((file) =>
          formData.append(mode === "edit" ? "newImages" : "images", file)
        );
        ...
        showToast(
          `Produit ${mode === "edit" ? "modifié" : "ajouté"} avec succès !`,
          "success"
        );
        setTimeout(() => {
          closeModal();
